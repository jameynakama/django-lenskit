...
# Run ai query tests — pass through extra pytest flags via `args:=...`
# Examples:
#  - just test-audit

# - just test-audit args="-q -k rule"
test-ai-query args="--cov=django_lenskit_ai_query --cov-report=term-missing":
    .venv/bin/python -m pytest {{ args }} --ds=django_lenskit_ai_query.tests.settings packages/django_lenskit_ai_query
